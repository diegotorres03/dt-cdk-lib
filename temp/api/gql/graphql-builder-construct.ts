...
export class GraphQLConstruct extends Construct {
  ...
  schema(schemaPath: string) {
    const definition = readFileSync(schemaPath).toString('utf8')
    new AppSync.CfnGraphQLSchema(this, 'gql-api-schema', {
      apiId: this.api.attrApiId,
      definition, //(await fs.readFile()).toString(),
    });
  ...
  private http = (type: string, fieldName: string, apiId: string) => (endpoint: { url: string; method?: string }) => {
    ...
    const httpDataSource = new AppSync.CfnDataSource(this, `http-data-source-${type}-${fieldName}`, {
      apiId: apiId,
    ...
    return next({}, (params: any) => {
      ...
      resolver.addDependsOn(httpDataSource);
  ...
  private fn = (type: string, fieldName: string, apiId: string) => (inlineFn: string, options?: any) => {
   ...
   *   }} options
   * @return {*} 
   * @memberof GraphQLConstruct
   */
  // createLambda(functionCode: string, options: {
  //   name: string; env: any; access: Function[]; vpc: EC2.Vpc; securityGroupIds: string[];
  // }) {

  createLambda(functionCode: string, name: string; env: any; access: Function[]; vpc: EC2.Vpc; securityGroupIds: string[]) {
    ...
    const functionCodeStr = functionCode
    let code;
  ...
  createLambdaRole(lambda: Lambda.Function, options?: any) {
  ...
  query(fieldName: string) {
    ...
    const pipe = (pipeContext: any = {}) => {
      ...
      const handlers = {
        fn: (inlineFn: string, options?: any) => {
          const lambda = this.createLambda(inlineFn, options);
  ...
  subscription(fieldName: string) {
    ...
    // const nextSteps = {
    //   setFilter: (mappingTemplate: string) => {
