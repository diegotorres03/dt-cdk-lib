...
export class ApiBuilderConstruct extends Construct {
    ...
    #apiHandlers: { [handlerName: string]: Lambda.Function } = {}
    ...
    requestHandler(method: string, path: string, lambdaCode: string, options: any) {

        const lambda = this.createLambda(lambdaCode, options);

        if (Array.isArray(options.layers)) { options.layers.forEach((layer: Lambda.LayerVersion) => lambda.addLayers(layer)); }
        ...
        this.#apiHandlers[`${method}/${path}`] = lambda;
    }

    post(path: string, lambdaCode: string, options: any) {
        this.requestHandler('POST', path, lambdaCode, options);
    }
    get(path: string, lambdaCode: string, options: any) {
        this.requestHandler('GET', path, lambdaCode, options);
    }
    delete(path: string, lambdaCode: string, options: any) {
        this.requestHandler('DELETE', path, lambdaCode, options);
    }
    patch(path: string, lambdaCode: string, options: any) {
        this.requestHandler('PATCH', path, lambdaCode, options);
    }
    put(path: string, lambdaCode: string, options: any) {
