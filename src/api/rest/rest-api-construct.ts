...
export class RestApiConstruct extends Construct {
    ...
    authorizer(name: string, handlerCode: string): RestApiConstruct {
        const authFn = new FunctionConstruct(this, `${this.api.restApiId}_${name}_authorizer`)
    ...
    private createMethodIntegration(method: string, path: string, handlerCode: string) {
        ...
        // [ ] deal with layers
        const options = this.currentAuthorizer ? {authorizer: this.currentAuthorizer} : undefined
        const integration = new ApiGateway.LambdaIntegration(fn.handlerFn); // {proxy: true}
        this.api.root.resourceForPath(path)
            .addMethod(method, integration, options);
